...
pub fn add_constant<C: CircuitContext>(
    ...
    for i in 0..a.len() {
        ...
        if i < first_one {
            ...
            circuit.add_gate(Gate::xor(a_i, TRUE_WIRE, wire));
            bits.push(wire);
            carry = Some(a_i);
        } else if b_bits[i] {
